...
def kissat_solve(iclauses, no_vars, tmp_filename=None, args=None, solver='kissat'):
    ...
    if args != None:
        cmd_solve = '{} {} {}'.format(solver, args, tmp_filename)
    else:
        cmd_solve = '{} {}'.format(solver, tmp_filename)
...
def kissat_solve_file(cnf_path, args=None, solver='kissat'):
    cnf, no_vars = read_cnf(cnf_path)
    return kissat_solve(cnf, no_vars, args=args, solver=solver)
...
def sort_cnf(cnf):
    ...
    def compare_literal(literal):
        return abs(literal)

    for clause in cnf:
        clause.sort(key=compare_literal)
